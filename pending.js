...
function exportOrderToExcel(order) {
    ...
    const sizeOrder = {
        ...
        '32B': 8, '32C': 9, '34B': 10, '34C': 11, '36B': 12, '36C': 13,
        '38B': 14, '38C': 15, '40B': 16, '40C': 17
    };

    // Collect all unique items, colors and sizes
    const items = new Set();
    ...
    const uniqueSizes = Array.from(sizes).sort((a, b) => {
    ...
    items.forEach(itemName => {
        const itemColors = order.items.filter(item => item.name === itemName);
        ...
        itemColors.forEach(item => {
            ...
            // Initialize all size columns to empty
            uniqueSizes.forEach(size => {
                row[size] = '';
            });
            
            // Fill in quantities where they exist
            if (item.quantities && typeof item.quantities === 'object') {
                Object.entries(item.quantities).forEach(([size, qty]) => {
                    if (qty > 0 && row.hasOwnProperty(size)) {
                        row[size] = qty;
                    }
                });
            }
    ...
    if (exportData.length === 1) { // Only header row
